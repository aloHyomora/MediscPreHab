...
class BuildCommand(Command):
    ...
    def run(self):
        ...
        for f in glob.glob(
            os.path.join(_NUGET_PATH, "**", "lib", "netstandard2.0", "*.dll"),
            recursive=True,
        ):
            basename = os.path.basename(f)
            self._copy(
                f, os.path.join(_BUILD_RUNTIME_PATH, "Plugins", "Protobuf", basename)
            )
        ...
        if self.command_args.android:
            ...
            self._copy(
                ...
                os.path.join(
                    _BUILD_RUNTIME_PATH, "Plugins", "Android", "mediapipe_android.aar"
                ),
            )
        ...
        if self.command_args.ios:
            ...
            self._unzip(
                self._find_latest_built_framework(),
                os.path.join(_BUILD_RUNTIME_PATH, "Plugins", "iOS"),
            )
        ...
        if self.command_args.analyzers:
            ...
            for f in glob.glob(
                os.path.join(_NUGET_PATH, "**", "analyzers", "dotnet", "cs", "*.dll"),
                recursive=True,
            ):
    ...
    def _build_common_commands(self):
        ...
        if self.command_args.android_ndk_api_level:
            commands += [
                "--action_env",
                f'ANDROID_NDK_API_LEVEL="{self.command_args.android_ndk_api_level}"',
            ]

        if self._is_windows():
            python_bin_path = (
                "C:\\Python312\\python.exe"  # sys.executable.replace('\\', '//')
            )
    ...
    def _build_android_commands(self):
        ...
        if self.command_args.android_fat_apk_cpu is not None:
            commands.append(
                f'--fat_apk_cpu={",".join(self.command_args.android_fat_apk_cpu)}'
            )

        commands.append(
            "//mediapipe_api/java/com/github/homuler/mediapipe:mediapipe_android"
        )
        return commands
...
class UninstallCommand(Command):
    ...
    def run(self):
        ...
        if self.command_args.desktop:
            ...
            for f in glob.glob(
                os.path.join(_INSTALL_PATH, "Runtime", "Plugins", "*"), recursive=True
            ):
        ...
        if self.command_args.android:
            ...
            aar_path = os.path.join(
                _INSTALL_PATH, "Runtime", "Plugins", "Android", "mediapipe_android.aar"
            )
        ...
        if self.command_args.ios:
            ...
            ios_framework_path = os.path.join(
                _INSTALL_PATH, "Runtime", "Plugins", "iOS", "MediaPipeUnity.framework"
            )
        ...
        if self.command_args.resources:
            ...
            for f in glob.glob(
                os.path.join(_INSTALL_PATH, "PackageResources", "MediaPipe", "*"),
                recursive=True,
            ):
            ...
            for f in glob.glob(
                os.path.join(_STREAMING_ASSETS_PATH, "*"), recursive=False
            ):
                self._remove(f)
        ...
        if self.command_args.protobuf:
            ...
            for f in glob.glob(
                os.path.join(_INSTALL_PATH, "Runtime", "Plugins", "Protobuf", "*.dll"),
                recursive=True,
            ):
                self._remove(f)
            ...
            for f in glob.glob(
                os.path.join(
                    _INSTALL_PATH, "Runtime", "Scripts", "Protobuf", "**", "*.cs"
                ),
                recursive=True,
            ):
                self._remove(f)
...
class Argument:
    ...
    def __init__(self):
        ...
        build_command_parser = subparsers.add_parser(
            "build", help="Build and install native libraries"
        )
        build_command_parser.add_argument("--desktop", choices=["cpu", "gpu"])
        build_command_parser.add_argument(
            "--android", choices=["armv7", "arm64", "x86", "x86_64"]
        )
        build_command_parser.add_argument("--android_fat_apk_cpu", nargs="+")
        build_command_parser.add_argument(
            "--android_ndk_api_level", type=int, choices=range(16, 31)
        )
        build_command_parser.add_argument("--ios", choices=["arm64"])
        build_command_parser.add_argument(
            "--resources", action=argparse.BooleanOptionalAction, default=True
        )
        build_command_parser.add_argument(
            "--analyzers",
            action=argparse.BooleanOptionalAction,
            default=False,
            help="Install Roslyn Analyzers",
        )
        build_command_parser.add_argument(
            "--compilation_mode",
            "-c",
        ...
        build_command_parser.add_argument(
            ...
            choices=["local", "cmake"],
            default="local",
            help="Decide to which OpenCV to link for Desktop native libraries",
        )
        build_command_parser.add_argument(
            "--solutions",
            nargs="+",
        ...
        build_command_parser.add_argument(
            "--linkopt", "-l", action="append", help="Linker options"
        )
        build_command_parser.add_argument(
            "--apple_bitcode",
            action=argparse.BooleanOptionalAction,
            default=True,
            help="Embed bitcode to iOS Framework",
        )
        build_command_parser.add_argument(
            "--macos_universal",
            action=argparse.BooleanOptionalAction,
            default=False,
            help="Build a universal library",
        )
        build_command_parser.add_argument(
            "--bazel_startup_opts", action="append", help="Bazel startup options"
        )
        build_command_parser.add_argument(
            "--bazel_build_opts", action="append", help="Bazel startup options"
        )
        ...
        clean_command_parser.add_argument(
            "--bazel_startup_opts", action="append", help="Bazel startup options"
        )
        clean_command_parser.add_argument("--verbose", "-v", action="count", default=0)

        uninstall_command_parser = subparsers.add_parser(
            "uninstall", help="Remove installed files"
        )
        uninstall_command_parser.add_argument(
            "--desktop", action=argparse.BooleanOptionalAction, default=True
        )
        uninstall_command_parser.add_argument(
            "--android", action=argparse.BooleanOptionalAction, default=True
        )
        uninstall_command_parser.add_argument(
            "--ios", action=argparse.BooleanOptionalAction, default=True
        )
        uninstall_command_parser.add_argument(
            "--resources", action=argparse.BooleanOptionalAction, default=True
        )
        uninstall_command_parser.add_argument(
            "--protobuf", action=argparse.BooleanOptionalAction, default=True
        )
        uninstall_command_parser.add_argument(
            "--analyzers", action=argparse.BooleanOptionalAction, default=True
        )
        uninstall_command_parser.add_argument(
